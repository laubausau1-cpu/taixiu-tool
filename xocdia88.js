...
class PredictionEngine{
    constructor(){
        ...
        this.weights=[0.35,0.35,0.15,0.15];this.nnBias=0.5;this.nnLR=0.01;
    ...
    calibrateFromHistory(){for(const log of this.predictionLog){if((log.danh_gia==='✅ ĐÚNG'||log.danh_gia==='❌ SAI')&&log.patterns&&log.patterns.length>0){const mp=log.patterns[0];if(!this.patternSuccessCount[mp])this.patternSuccessCount[mp]=0;if(!this.patternFailCount[mp])this.patternFailCount[mp]=0;if(log.danh_gia==='✅ ĐÚNG')this.patternSuccessCount[mp]++;else this.patternFailCount[mp]++}}for(const name in this.patternSuccessCount){const s=this.patternSuccessCount[name]||0,f=this.patternFailCount[name]||0;if(s+f>=3)this.patternWeights[name]=clamp(s/(s+f),0.2,2.0)}}
    ...
    adjustPatternScore(name,base){if(this.patternWeights[name]){const adj=base*this.patternWeights[name];if(Math.abs(adj-base)>0.05)this.stats.adaptiveCorrections++;return clamp(adj,0.05,0.95)}return base}
    addResult(resultInput,sessionData={}){
        ...
        if(matchedLog){const predictedResult=matchedLog.du_doan==='Tài'?'T':'X';matchedLog.ket_qua_thuc_te=actualResult==='T'?'Tài':'Xỉu';if(predictedResult===actualResult){matchedLog.danh_gia='✅ ĐÚNG';this.stats.correctPredictions++;const conf=parseInt(matchedLog.do_tin_cay);if(conf>=70){this.stats.highConfCorrect++;this.stats.highConfTotal++}if(matchedLog.patterns&&matchedLog.patterns.length>0){const mp=matchedLog.patterns[0];if(!this.patternSuccessCount[mp])this.patternSuccessCount[mp]=0;this.patternSuccessCount[mp]++}}else{matchedLog.danh_gia='❌ SAI';this.stats.wrongPredictions++;const conf=parseInt(matchedLog.do_tin_cay);if(conf>=70)this.stats.highConfTotal++;if(matchedLog.patterns&&matchedLog.patterns.length>0){const mp=matchedLog.patterns[0];if(!this.patternFailCount[mp])this.patternFailCount[mp]=0;this.patternFailCount[mp]++}}if(matchedLog.patterns&&matchedLog.patterns.length>0){const mp=matchedLog.patterns[0],s=this.patternSuccessCount[mp]||0,f=this.patternFailCount[mp]||0;if(s+f>=3)this.patternWeights[mp]=clamp(s/(s+f),0.2,2.0)}}
        ...
        if(actualResult==='T'){this.stats.totalTai++;if(this.stats.currentStreakCount>this.stats.longestTaiStreak)this.stats.longestTaiStreak=this.stats.currentStreakCount}else{this.stats.totalXiu++;if(this.stats.currentStreakCount>this.stats.longestXiuStreak)this.stats.longestXiuStreak=this.stats.currentStreakCount}
        this.stats.totalSessions++;this.history.push(actualResult);this.sessions.push({id:currentSessionId,result:actualResult,time:new Date().toISOString(),dice:sessionData.dice||null,total:sessionData.total||null,betTai:sessionData.betTai||0,betXiu:sessionData.betXiu||0});
        if(this.history.length>MAX_HISTORY)this.history=this.history.slice(-MAX_HISTORY);this.lastResult=actualResult;this.savePredictionLog();return actualResult;
    }
    ...
    P48(){if(this.history.length<8)return null;const l8=this.getLastElements(8),f7=l8.slice(0,7),tc=this.countOccurrences(f7,'T');if(tc>=6&&l8[7]==='X')return this.createPatternResult('48. Bet Tai gay->Tai','T',0.68);if(tc<=1&&l8[7]==='T')return this.createPatternResult('48. Bet Xiu gay->Xiu','X',0.68);return null}
    P49(){if(this.sessions.length<5)return null;const totals=this.getRecentTotals(5);if(totals.length<5)return null;const avg=totals.reduce((a,b)=>a+b,0)/5;if(avg>12)return this.createPatternResult('49. Tong cao->Tai','T',0.62);if(avg<9)return this.createPatternResult('49. Tong thap->Xiu','X',0.62);return null}
    P50(){if(this.sessions.length<5)return null;let tb=0,xb=0;for(let i=this.sessions.length-5;i<this.sessions.length;i++){tb+=this.sessions[i].betTai||0;xb+=this.sessions[i].betXiu||0}if(tb+xb>0){const r=tb/(tb+xb);if(r>0.65)return this.createPatternResult('50. Cuoc lech->Xiu','X',0.58);if(r<0.35)return this.createPatternResult('50. Cuoc lech->Tai','T',0.58)}return null}
    P51(){if(this.history.length<6)return null;const l6=this.getLastElements(6);if(l6.slice(0,3).join('')===[...l6.slice(3,6)].reverse().join(''))return this.createPatternResult('51. Mirror','T',l6[5]==='T'?'X':'T',0.71);return null}
    P52(){if(this.predictionLog.length<5)return null;const recent=this.predictionLog.slice(-5);const wrongs=recent.filter(l=>l.danh_gia==='❌ SAI');if(wrongs.length>=4)return this.createPatternResult('52. Reverse',this.lastPrediction==='T'?'X':'T',0.55);return null}
    P53(){if(this.history.length<7)return null;const patterns=this.analyzeAllPatterns();const top3=patterns.slice(0,3);if(top3.length>=3&&top3.every(p=>p.prediction===top3[0].prediction))return this.createPatternResult('53. Triple',top3[0].prediction,0.76);return null}
    ...
    analyzeAllPatterns(){return [this.P01(),this.P02(),this.P07(),this.P08(),this.P09(),this.P10(),this.P16(),this.P19(),this.P20(),this.P21(),this.P26(),this.P30(),this.P31(),this.P34(),this.P47(),this.P48(),this.P49(),this.P50(),this.P51(),this.P52(),this.P53(),this.P54()].filter(p=>p!==null)}
...
console.log('TAI XIU TOOL V8.0 - 22 PATTERNS - HIGH CONF');
ensureDirectories();
connectWebSocket();
