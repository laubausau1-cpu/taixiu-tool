...
class PredictionEngine{
    ...
    predict(){
        if(this.history.length<MIN_SAMPLES){const lr=this.history.length>0?this.history[this.history.length-1]:'T';this.lastPrediction=lr==='T'?'X':'T';return{prediction:this.lastPrediction,confidence:50,patterns:[{name:'Waiting...',prediction:this.lastPrediction,score:0.5}],method:'default',adaptive:false};}
...
console.log('TAI XIU TOOL V8.0 - 22 PATTERNS');
ensureDirectories();
connectWebSocket();
