...
function connect(){
  ...
  ws.on('message',raw=>{try{const msg=JSON.parse(raw.toString());if(!msg.M)return;for(const m of msg.M){if(m.M==='Md5sessionInfo'){const s=m.A[0];if(s.CurrentState===0&&s.Ellapsed>0)process.stdout.write('\r⏳'+s.Ellapsed+'s | 🎯'+engine.getAccuracy()+'% | 📊'+engine.stats.total+'   '); | 🎯'+engine.getAccuracy()+'% | 📊'+engine.stats.total+'   ');if(s.CurrentState===1&&s.Result&&s.Result.Dice1>0&&engine.lastSid!==s.SessionID){engine.lastSid=s.SessionID;const d1=s.Result.Dice1,d2=s.Result.Dice2,d3=s.Result.Dice3,total=d1+d2+d3,result=total>=11?'Tài':'Xỉu';const p=engine.predict();engine.logPrediction(s.SessionID+1,p);engine.addResult(result,{sessionId:s.SessionID,total,dice:d1+'-'+d2+'-'+d3});console.log('\n┌──────────────────────────────────────────┐');console.log('│ #'+s.SessionID+' | 🎲['+d1+','+d2+','+d3+']='+total+' | '+result);console.log('│ 🎯 '+engine.getAccuracy()+'% | 📊 '+engine.stats.total);console.log('├──────────────────────────────────────────┤');console.log('│ 🔮 DỰ ĐOÁN: '+p.prediction+' ('+p.confidence+'%)');console.log('│ 💡 '+p.reason);console.log('└──────────────────────────────────────────┘\n');}}}}catch(_){}});
